    ...
    implements ZFDebug_Controller_Plugin_Debug_Plugin_Interface
{
    ...
    public function mark($name, $logFirst = false) {
        if (isset($this->_marks[$name])) {
            $this->_marks[$name]['time'] = (microtime(true)-$_SERVER['REQUEST_TIME_FLOAT'])*1000-$this->_marks[$name]['time'];
            $this->_marks[$name]['memory'] = memory_get_usage()-$this->_marks[$name]['memory'];
            $this->_logger->zflog(
